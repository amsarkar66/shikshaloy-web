...
function StatsRow({ events }: { events: SchoolEvent[] }) {
...
function DayPanel({ dateStr, events, onOpenPtm }: { dateStr: string; events: SchoolEvent[]; onOpenPtm: (id: string) => void }) {
  const today   = todayStr();
      ...
      <div className="flex-1 overflow-y-auto divide-y divide-gray-100 dark:divide-zinc-700/50">
        {dayEvents.length === 0 ? (
          ...
          dayEvents.map((ev) => {
            ...
            const clickable = Boolean(ev.ptmSessionId);
            return (
              <div
                key={ev.id}
                onClick={() => ev.ptmSessionId && onOpenPtm(ev.ptmSessionId)}
                className={`px-4 py-3 space-y-1.5 ${clickable ? "cursor-pointer hover:bg-gray-50 dark:hover:bg-zinc-700/30" : ""}`}
              >
...
function CalendarView({ events, onOpenPtm }: { events: SchoolEvent[]; onOpenPtm: (id: string) => void }) {
  const today = todayStr();
  ...
  const eventsByDate = useMemo(() => {
    ...
    cells.forEach(({ date }) => { map[date] = getEventsForDate(events, date); });
    return map;
  }, [cells, events]);
  ...
  return (
    <div className="flex gap-4">
      ...
      {/* Day panel */}
      <DayPanel dateStr={selectedDate} events={events} onOpenPtm={onOpenPtm} />
    </div>
  );
}
...
function EventCard({ event, onOpenPtm }: { event: SchoolEvent; onOpenPtm: (id: string) => void }) {
...
function ListView({ events, onOpenPtm }: { events: SchoolEvent[]; onOpenPtm: (id: string) => void }) {
  const today = todayStr();
  const [typeFilter, setTypeFilter] = useState<EventType | "all">("all");
  const [timeFilter, setTimeFilter] = useState<"upcoming" | "past" | "all">("upcoming");
  const [page,       setPage]       = useState(1);

  const timeFiltered = useMemo(() => events.filter((e) => {
    const end = e.endDate ?? e.date;
    ...
    <div className="space-y-4">
      ...
      {pageData.length === 0 ? (
        ...
        <div className="space-y-3">
          {pageData.map((event) => <EventCard key={event.id} event={event} onOpenPtm={onOpenPtm} />)}
        </div>
      )}
...
export default function EventsClient({
  initialEvents, initialSessions, sections, teachers,
}: {
  initialEvents: SchoolEvent[];
  initialSessions: PtmSession[];
  sections: Section[];
  teachers: Teacher[];
}) {
  ...
  return (
    <div className="w-full px-6 py-6 space-y-5">
      ...
      {tab === "calendar" ? (
        <CalendarView events={events} onOpenPtm={setViewingSessionId} />
      ) : (
        <ListView events={events} onOpenPtm={setViewingSessionId} />
      )}
