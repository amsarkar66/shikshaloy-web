...
function StatsRow({ items }: { items: InventoryItem[] }) {
  const totalItems = items.length;
  const totalQty = items.reduce((s, i) => s + i.totalQty, 0);
  const totalDamaged = items.reduce((s, i) => s + i.damaged, 0);
  const inventoryVal = totalValue(items);

  const stats = [
    { label: "Item Types",       value: String(totalItems),          icon: Package,      accent: "text-indigo-500  bg-indigo-500/10"  },
    { label: "Total Units",      value: String(totalQty),             icon: PackageCheck, accent: "text-blue-500    bg-blue-500/10"    },
    { label: "Damaged / Faulty", value: String(totalDamaged),         icon: Wrench,       accent: "text-amber-500   bg-amber-500/10"   },
    { label: "Total Value",      value: formatCurrency(inventoryVal), icon: IndianRupee,  accent: "text-emerald-500 bg-emerald-500/10" },
  ];
    ...
    <div className="w-full px-6 py-6 space-y-5">
      ...
      >
        <TableHead>
          ...
          <Th>Condition</Th>
          <Th>Status</Th>
          <Th><button onClick={() => toggleSort("value")} className="flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wider text-gray-500 dark:text-zinc-400 hover:text-gray-900 dark:hover:text-zinc-100 transition-colors">Value <SortIcon active={sortField === "value"} dir={sortDir} /></button></Th>
