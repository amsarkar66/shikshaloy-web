...
function StatsRow({
  ...
  const stats = tt ? computeStats(tt, periods) : null;
  ...
  const items = isSummary
    ...
    : [
        ...
        { label: viewMode === "teacher" ? "Free Periods" : "Free Slots", value: stats ? `${stats.free}` : "—", sub: viewMode === "teacher" ? "non-teaching periods" : "unscheduled periods", icon: Clock, accent: "text-amber-500  bg-amber-500/10" },
        { label: "Subjects", value: stats ? `${stats.subjects}` : "—", sub: "unique subjects", icon: BookOpen, accent: "text-emerald-500 bg-emerald-500/10" },
        { label: viewMode === "teacher" ? "Classes" : "Teachers", value: stats ? `${stats.teachers}` : "—", sub: viewMode === "teacher" ? "classes assigned" : "assigned teachers", icon: viewMode === "teacher" ? Layers : Users, accent: "text-blue-500   bg-blue-500/10" },
      ];
...
function TimetableGrid({ tt, rowItems }: { tt: ClassTimetable; rowItems: RowItem[] }) {
...
function TeacherSummaryTable({ summaries, onSelect }: { summaries: TeacherSummary[]; onSelect: (teacher: string) => void }) {
  if (summaries.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center gap-2 rounded-xl border border-dashed border-gray-200 dark:border-zinc-700 bg-white dark:bg-zinc-800/30 py-24">
        ...
        <p className="text-sm font-medium text-gray-500 dark:text-zinc-400">No teacher timetable data yet</p>
        <p className="text-xs text-gray-400 dark:text-zinc-500">Assign teachers to timetable slots to see workload here.</p>
      </div>
    );
  }
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-200 dark:border-zinc-700 bg-gray-50 dark:bg-zinc-800">
              <th className="py-3 pl-4 pr-3 text-left text-[11px] font-semibold uppercase tracking-wider text-gray-500 dark:text-zinc-400">Teacher</th>
              <th className="px-3 py-3 text-left text-[11px] font-semibold uppercase tracking-wider text-gray-500 dark:text-zinc-400">Subjects</th>
              <th className="px-3 py-3 text-left text-[11px] font-semibold uppercase tracking-wider text-gray-500 dark:text-zinc-400">Classes</th>
              <th className="px-3 py-3 text-left text-[11px] font-semibold uppercase tracking-wider text-gray-500 dark:text-zinc-400">Periods / wk</th>
              <th className="py-3 pl-3 pr-4 text-right text-[11px] font-semibold uppercase tracking-wider text-gray-500 dark:text-zinc-400">Schedule</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100 dark:divide-zinc-700/50">
            {summaries.map((t) => (
              <tr key={t.teacher} onClick={() => onSelect(t.teacher)} className="hover:bg-gray-50 dark:hover:bg-zinc-700/30 transition-colors cursor-pointer">
                <td className="py-3 pl-4 pr-3">
                  <div className="flex items-center gap-3">
                    <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-[11px] font-bold text-white ${avatarColor(t.teacher)}`}>{initials(t.teacher)}</div>
                ...
                <td className="px-3 py-3">
                  <div className="flex flex-wrap gap-1">
                    {t.classes.slice(0, 4).map((c) => <span key={c} className="inline-flex items-center rounded-md bg-gray-100 dark:bg-zinc-700 px-2 py-0.5 text-[11px] font-semibold text-gray-600 dark:text-zinc-300">{c}</span>)}
...
export default function TimetableClient({
  ...
  function handleViewChange(v: ViewMode) {
    setView(v);
    if (v === "teacher") setTeacher("");
  }

  const activeTt: ClassTimetable | null = useMemo(() => {
    if (view === "class") return timetables[selClass] ?? null;
    if (!selTeacher) return null;
    return getTeacherSchedule(timetables, selTeacher);
  }, [view, selClass, selTeacher, timetables]);

  const hasAnySlot = (tt: ClassTimetable | null) => tt ? Object.values(tt).some((day) => Object.keys(day).length > 0) : false;

  const configuredCount = Object.values(timetables).filter((tt) => hasAnySlot(tt)).length;
