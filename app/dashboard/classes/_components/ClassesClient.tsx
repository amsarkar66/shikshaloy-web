...
function StatsRow({ sections }: { sections: ClassSection[] }) {
  ...
  const items = [
    { label: "Total Classes",    value: classNums.length,  icon: GraduationCap, accent: "text-indigo-500  bg-indigo-500/10"  },
    { label: "Total Sections",   value: sections.length,   icon: Layers,        accent: "text-violet-500  bg-violet-500/10"  },
    ...
    <div className="rounded-xl border border-gray-200 dark:border-zinc-800 bg-white dark:bg-zinc-800/50 p-5 flex flex-col gap-4 hover:border-indigo-300 dark:hover:border-indigo-700 transition-colors group">
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-indigo-500/10 dark:bg-indigo-500/20">
            <span className="text-sm font-bold text-indigo-600 dark:text-indigo-400">{sec.classNum}<span className="text-xs">–{sec.section}</span></span>
          </div>
          <div>
        ...
        <span className={`shrink-0 text-[10px] font-semibold px-2 py-0.5 rounded-full ${sec.status==="active"?"bg-emerald-500/10 text-emerald-600 dark:text-emerald-400":"bg-zinc-100 dark:bg-zinc-700 text-zinc-500 dark:text-zinc-400"}`}>
          {sec.status==="active"?"Active":"Inactive"}
        </span>
      </div>
      <div className="flex items-center gap-2">
        <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-gray-100 dark:bg-zinc-700"><GraduationCap className="h-3 w-3 text-gray-500 dark:text-zinc-400"/></div>
      ...
      <div className="flex items-center justify-between pt-1 border-t border-gray-100 dark:border-zinc-700/50">
        ...
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          <button className="flex h-7 items-center gap-1 rounded-lg px-2 text-xs font-medium text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 transition-colors">Roster <ChevronRight className="h-3 w-3"/></button>
        </div>
      </div>
    </div>
  );
...
export default function ClassesClient({ initialSections }: { initialSections: ClassSection[] }) {
    ...
    <div className="w-full px-6 py-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          {["all", ...classNums].map((n) => (
            <button key={n} onClick={() => setClassFilter(n)} className={`h-9 rounded-lg px-3 text-sm font-medium transition-colors ${classFilter===n?"bg-indigo-500 text-white shadow-sm":"border border-gray-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-gray-600 dark:text-zinc-400 hover:text-gray-900 dark:hover:text-zinc-100"}`}>
              {n==="all"?"All":`Class ${n}`}
            </button>
          ))}
        </div>
        ...
        <button className="flex h-9 items-center gap-1.5 rounded-lg bg-indigo-500 hover:bg-indigo-600 px-4 text-sm font-medium text-white transition-colors shadow-sm sm:ml-auto shrink-0"><Plus className="h-4 w-4"/> Add Section</button>
      </div>
      <p className="text-xs text-gray-500 dark:text-zinc-500 -mt-2">Showing <span className="font-medium text-gray-700 dark:text-zinc-300">{filtered.length}</span> of <span className="font-medium text-gray-700 dark:text-zinc-300">{initialSections.length}</span> sections{hasFilter&&<span className="ml-2 text-indigo-600 dark:text-indigo-400 font-medium">· Filters active</span>}</p>
      {visibleClasses.length===0?(
        <div className="flex flex-col items-center justify-center gap-2 rounded-xl border border-dashed border-gray-200 dark:border-zinc-700 bg-white dark:bg-zinc-800/30 py-20">
          <Layers className="h-8 w-8 text-gray-300 dark:text-zinc-600"/><p className="text-sm font-medium text-gray-500 dark:text-zinc-400">No sections found</p><p className="text-xs text-gray-400 dark:text-zinc-500">Try adjusting your search or filters</p>
        </div>
      ):(
            ...
            return (
              <div key={classNum}>
                <div className="flex items-center gap-3 mb-4">
                  <div className="flex items-center gap-2">
                    <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-indigo-500 shadow-sm shadow-indigo-500/30"><span className="text-xs font-bold text-white">{classNum}</span></div>
