...
export function AddSectionModal({ open, onClose, onCreated, teachers, streams = [], lockedClassNum }: AddSectionModalProps) {
  const [classNum, setClassNum] = useState(lockedClassNum ?? "");
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!classNum.trim() || !section.trim()) {
      setError("Class number and section name are required.");
      return;
    }
    setBusy(true);
    setError(null);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={handleClose}>
      ...
      >
        <div className="flex items-center justify-between border-b border-gray-200 dark:border-zinc-800 px-5 py-4">
          <p className="text-sm font-semibold text-gray-900 dark:text-zinc-50">Add Section</p>
