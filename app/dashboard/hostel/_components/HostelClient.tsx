...
function RoomsTab({ rooms, blocks }: { rooms: HostelRoom[]; blocks: string[] }) {
        ...
        {blocks.map((block) => {
          ...
          return (
            <div key={block} className="rounded-xl border border-gray-200 dark:border-zinc-800 bg-white dark:bg-zinc-800/50 p-4">
              ...
              <div className="h-1.5 rounded-full bg-gray-100 dark:bg-zinc-700 mb-2"><div className={`h-1.5 rounded-full transition-all ${pct >= 90 ? "bg-amber-500" : "bg-indigo-500"}`} style={{ width: `${pct}%` }} /></div>
              <p className="text-[10px] text-gray-400 dark:text-zinc-500 truncate">{blockRooms.length} room{blockRooms.length === 1 ? "" : "s"}</p>
            </div>
          );
        })}
      </div>
...
function StudentsTab({ students, blocks }: { students: HostelStudent[]; blocks: string[] }) {
  const [query, setQuery] = useState("");
  const [blockFilter, setBlock] = useState("all");
...
export default function HostelClient({
  ...
  return (
    <div className="w-full px-6 py-6 space-y-5">
      ...
      {activeTab === "rooms" && <RoomsTab rooms={rooms} blocks={blocks} />}
      {activeTab === "students" && <StudentsTab students={students} blocks={blocks} />}
    </div>
  );
}
