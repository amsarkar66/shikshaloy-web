...
import {
  bookStatus, availableCopies, avatarColor, initials,
...
export default function LibraryClient({ books }: { books: Book[] }) {
    ...
    <div className="w-full px-6 py-6 space-y-5">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex gap-2 sm:ml-auto">
          ...
          <button className="flex h-9 items-center gap-1.5 rounded-lg bg-indigo-500 hover:bg-indigo-600 px-4 text-sm font-medium text-white transition-colors shadow-sm"><Plus className="h-4 w-4" /> Add Book</button>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between">
        <p className="text-xs text-gray-500 dark:text-zinc-500">
          ...
          <span className="font-medium text-gray-700 dark:text-zinc-300">{books.length}</span> titles
        </p>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 dark:border-zinc-700 bg-gray-50 dark:bg-zinc-800">
                ...
                <th className="px-3 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-zinc-400">Copies</th>
                <th className="px-3 py-3 text-left">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-zinc-700/50">
              {pageData.length === 0 ? (
                <tr><td colSpan={8} className="py-20 text-center"><div className="flex flex-col items-center gap-2"><BookX className="h-8 w-8 text-gray-300 dark:text-zinc-600" /><p className="text-sm font-medium text-gray-500 dark:text-zinc-400">No books found</p><p className="text-xs text-gray-400 dark:text-zinc-500">Try adjusting your search or filters</p></div></td></tr>
              ) : pageData.map((book) => {
                ...
                return (
                  <tr key={book.id} className="hover:bg-gray-50 dark:hover:bg-zinc-700/30 transition-colors">
                    ...
                    <td className="px-3 py-3"><span className="inline-flex items-center rounded-lg bg-indigo-500/10 px-2.5 py-1 text-xs font-semibold text-indigo-700 dark:text-indigo-300">{book.category}</span></td>
                    <td className="px-3 py-3"><span className="text-sm font-medium text-gray-700 dark:text-zinc-300 tabular-nums">{book.totalCopies}</span></td>
                    ...
                    <td className="px-3 py-3"><span className="text-sm text-gray-700 dark:text-zinc-300 tabular-nums">{book.issued}</span></td>
                    <td className="px-3 py-3">
                      {book.overdue > 0 ? (
                    ...
                    <td className="px-3 py-3"><span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-medium ${badge.cls}`}>{badge.label}</span></td>
