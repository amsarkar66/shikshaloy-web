...
export default function StudentsClient({ students: initialStudents }: { students: Student[] }) {
  ...
  }, [query, classFilter, feeFilter, sortField, sortDir, students]);

  const totalPages = Math.max(1, Math.ceil(filtered.length / PAGE_SIZE));
  const pageData   = filtered.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE);

  function clearFilters() { setQuery(""); setClass("all"); setFee("all"); setPage(1); }
                  ...
                  <tr key={s.id} className="hover:bg-gray-50 dark:hover:bg-zinc-700/30 transition-colors">
                    ...
                    <td className="py-3 pl-3 pr-4">
                      <div className="flex items-center justify-end gap-1">
                        <Link href={`/dashboard/students/${s.id}`} className="flex h-7 w-7 items-center justify-center rounded-lg text-gray-400 dark:text-zinc-500 hover:bg-gray-100 dark:hover:bg-zinc-700 hover:text-gray-700 dark:hover:text-zinc-200 transition-colors">
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1)
                ...
                .map((n, i) =>
                  n === "…" ? (
                    ...
                    <button key={n} onClick={() => setPage(n as number)} className={`flex h-7 w-7 items-center justify-center rounded-lg text-xs font-medium transition-colors ${page === n ? "bg-indigo-500 text-white" : "border border-gray-200 dark:border-zinc-700 text-gray-600 dark:text-zinc-400 hover:bg-gray-100 dark:hover:bg-zinc-700"}`}>
                      {n}
                    </button>
                  )
                )}
