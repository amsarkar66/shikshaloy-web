...
Status: **All five phases shipped, including the scoping fix** (§7, §9, §10). Phase 3 landed as "stop writing `profiles.role = 'admin'` on promotion," not a literal enum collapse — see §9 for why that's the actual correct shape. Multi-school (§5) remains explicitly deferred — everything shipped is same-school only.
...
**The §9 limitation — fixed 2026-09-12, commit `a69d640`.** Re-checking the three named spots turned up two real bugs (not just under-scoping) and one false alarm:
- `subjects/attendance-actions.ts` — confirmed bug. `requireSubjectAttendanceMarker()` went through `requireRole(["admin","super_admin","teacher"])`, whose allowed list already contains `"teacher"` — a promoted teacher's literal role satisfied the check directly and never reached `requireRole`'s own admin-grant fallback, so they got restricted `teacherProfileId` instead of unrestricted. Fixed by exporting `hasAdminGrant(profileId)` (previously private inside `isAdmin()`) and checking it directly alongside the role comparison.
- `homework/[id]/page.tsx` — worse than documented: an actual `notFound()`, not just reduced scope. `canEdit` was already correctly `true` for an admin-grant teacher, but the very next `if (role === "teacher")` block ran unconditionally and called `notFound()` for any homework they didn't personally assign — discarding the correct `canEdit` value entirely before it could matter. Fixed by skipping that block when `userIsAdmin`.
- `grades/actions.ts` marks-entry — **false alarm.** Re-read it and it already calls `isAdmin(vu)` unconditionally, not gated behind a role-array short-circuit like the other two — it never had this problem. Listed in §9 without re-verifying at the time.
