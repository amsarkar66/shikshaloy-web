-- Grievances demo data: the public demo school had no entries in the
-- Grievances inbox, so the page looked empty for demo visitors. This seeds
-- it once now, and folds grievance activity into reset_demo_school() so it's
-- regenerated fresh (relative to "today", like front-desk logs) on every
-- nightly reset instead of going stale or disappearing.
...
declare
  ...
  v_admin_id  uuid := 'a88700c8-e59c-4d68-aeab-b17a37e3bfcd'; -- admin@shikshaloy.com
begin
  insert into grievances (school_id, name, email, phone, category, subject, message, status, resolution_notes, resolved_by, created_at) values
    (v_school_id, 'Rahim Uddin', 'rahim.uddin@example.com', '+8801711000111', 'academic', 'Extra homework load in Class 8',
    ...
    (v_school_id, 'Farzana Akter', 'farzana.akter@example.com', '+8801711000222', 'transport', 'School bus arriving 20 minutes late',
     ...
     'in_review', null, null, now() - interval '1 day 4 hours'),
    (v_school_id, 'Kamal Hossain', 'kamal.hossain@example.com', '+8801711000333', 'facilities', 'Broken fan in Class 6 section B',
     ...
     'resolved', 'Fan was replaced by the maintenance team on the same day this was reported.', v_admin_id, now() - interval '5 days'),
    (v_school_id, 'Nasrin Sultana', 'nasrin.sultana@example.com', '+8801711000444', 'fees', 'Discrepancy in the fee receipt amount',
    ...
    (v_school_id, 'Abdul Karim', 'abdul.karim@example.com', '+8801711000555', 'staff', 'Concern about a substitute teacher''s conduct',
     ...
     'in_review', null, null, now() - interval '3 days'),
    (v_school_id, 'Shirin Begum', null, '+8801711000666', 'other', 'Request for a parent-teacher meeting slot',
     ...
     'resolved', 'Meeting scheduled and completed with the class teacher on the following Saturday.', v_admin_id, now() - interval '9 days');
end $$;

-- ============================================================
-- 2. Fold grievance regeneration into the nightly reset function
-- ============================================================
...
begin
  ...
  -------------------------------------------------------------------
  delete from grievances where school_id = v_school_id;
  ...
  -- Grievances: parent/public complaints inbox — regenerated fresh each
  -- reset (not baseline data) so the page never looks empty for demo visitors.
  insert into grievances (school_id, name, email, phone, category, subject, message, status, resolution_notes, resolved_by, created_at) values
    (v_school_id, 'Rahim Uddin', 'rahim.uddin@example.com', '+8801711000111', 'academic', 'Extra homework load in Class 8',
     'My son is getting an unusually heavy amount of homework from the science teacher every day, leaving almost no time for other subjects. Could this be reviewed and balanced across subjects?',
     'open', null, null, now() - interval '6 hours'),
