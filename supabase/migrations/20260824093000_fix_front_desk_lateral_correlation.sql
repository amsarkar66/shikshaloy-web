-- The front-desk LATERAL subqueries added in 20260824090000 (picking a
-- random staff member for visitor_logs.meeting_with, and a random student
-- for gate_passes.student_id) didn't reference any column from their outer
-- row, so Postgres treated them as uncorrelated and evaluated them ONCE per
-- query rather than once per row — every visitor ended up "meeting" the
-- same staff member, and every gate pass referenced the same student.
-- Fix: order by md5(id || <a per-row-unique outer column>) instead of
-- order by random(), which forces genuine per-row correlation (and is
-- deterministic-but-varied, consistent with the md5-seeding style already
-- used elsewhere in this function for attendance).
...
begin
  ...
  --
  -- The lateral joins below order by md5(id || <per-row-unique outer col>)
  -- rather than random() — ordering by a value correlated to the outer row
  -- forces Postgres to evaluate the subquery once per row instead of
  -- hoisting it as a constant, which is what a plain `order by random()`
  -- (no outer-row reference) was silently doing here.
  ...
  join lateral (
    select full_name from staff_members where school_id = v_school_id order by md5(id::text || v.name) limit 1
  ) sm on true;
  ...
  join lateral (
    select id from students where school_id = v_school_id and academic_year_id = v_ay_id order by md5(id::text || g.reason) limit 1
  ) st on true;
